-- W5 Implementation Migration: Reorder Enhancements
...
UPDATE products SET 
    safety_stock_days = 3 WHERE safety_stock_days IS NULL,
    pack_size = 1 WHERE pack_size IS NULL;
...
ALTER TABLE products 
ADD CONSTRAINT chk_products_safety_stock_days CHECK (safety_stock_days >= 0),
ADD CONSTRAINT chk_products_pack_size CHECK (pack_size >= 1),
ADD CONSTRAINT chk_products_max_stock_days CHECK (max_stock_days IS NULL OR max_stock_days > 0),
...
ADD CONSTRAINT chk_suppliers_moq CHECK (minimum_order_quantity >= 1);

-- Update existing supplier records to ensure data quality
UPDATE suppliers SET 
    lead_time_days = 7 WHERE lead_time_days IS NULL OR lead_time_days < 0,
    minimum_order_quantity = 1 WHERE minimum_order_quantity IS NULL OR minimum_order_quantity < 1;
...
-- Update contact_email to email if email doesn't exist but contact_email does
UPDATE suppliers SET email = contact_email WHERE email IS NULL AND contact_email IS NOT NULL;
UPDATE suppliers SET phone = contact_phone WHERE phone IS NULL AND contact_phone IS NOT NULL;
UPDATE suppliers SET is_active = 'true' WHERE is_active IS NULL;

-- Add constraint for is_active field
ALTER TABLE suppliers 
ADD CONSTRAINT chk_suppliers_is_active CHECK (is_active IN ('true', 'false'));
